...
public class AggressiveBot : BotTemplate
{
	public AggressiveBot(int botColor)
	{
		color = botColor;
	...
	public NextMove nextMove()
	{
		...
		foreach (NextMove nextMove in allMoves)
		{
			...
			List<Piece> piecesOnBoard2 = getPiecesOnBoardState(this.currentBoardState, this.color * -1);
			foreach (Piece item in piecesOnBoard2)
			{
				if (item.baseType == "King")
				{
			...
			bool inCheckOpp = isGuarded(this, this.currentBoardState, this.color, kingPosOpp);
			...
			foreach (NextMove nextMoveOpp in allMovesOpp)
			{
				...
				foreach (NextMove nextMove2 in allMoves2)
				{
					...
					coords kingPos2 = new coords(-1, -1);
					List<Piece> piecesOnBoard3 = getPiecesOnBoardState(this.currentBoardState, this.color);
					foreach (Piece item in piecesOnBoard3)
					{
						if (item.baseType == "King")
						{
							kingPos2 = item.position;
						}
					}

					bool inCheck2 = isGuarded(this, this.currentBoardState, this.color * -1, kingPos2);

					coords kingPosOpp2 = new coords(-1, -1);
					List<Piece> piecesOnBoard4 = getPiecesOnBoardState(this.currentBoardState, this.color * -1);
					foreach (Piece item in piecesOnBoard4)
					{
						if (item.baseType == "King")
						{
							kingPosOpp2 = item.position;
						}
					}

					bool inCheckOpp2 = isGuarded(this, this.currentBoardState, this.color, kingPosOpp2);
