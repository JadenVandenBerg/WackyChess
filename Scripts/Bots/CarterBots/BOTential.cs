...
public class BOTential : BotTemplate
{
	public BOTential(int botColor)
	{
		color = botColor;
	...
	public NextMove nextMove()
	{
		...
		foreach (NextMove nextMove in allMoves)
		{
			...
			float leastMovesAfterOpp = 100000;
			...
			foreach (NextMove nextMoveOpp in allMovesOpp)
			{
				...
				List<NextMove> allMovesNext = getAllPossibleBotMovesAndAbilities(this, cloneState_, this.color);

				if (allMovesNext.Count < leastMovesAfterOpp)
				{
					leastMovesAfterOpp = allMovesNext.Count;
					bestOppNextMove = nextMoveOpp;
				}
			}

			if (leastMovesAfterOpp >= bestNumMoves)
			{
				if (leastMovesAfterOpp > bestNumMoves)
				{
					validMoves.Clear();
				...
				bestNumMoves = leastMovesAfterOpp;
