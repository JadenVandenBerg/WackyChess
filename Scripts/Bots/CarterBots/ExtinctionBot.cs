...
public class ExtinctionBot : BotTemplate
{
	public ExtinctionBot(int botColor)
	{
		color = botColor;
	...
	public NextMove nextMove()
	{
		...
		foreach (NextMove nextMove in allMoves)
		{
			...
			float bestMoveTotalPoints = 1000000;
			...
			foreach (NextMove nextMoveOpp in allMovesOpp)
			{
				...
				float totalPointsOnBoard = botPoints + oppPoints;
				...
				if (diff < bestOppMoveDiff)
				{
					...
					bestMoveTotalPoints = totalPointsOnBoard;
				}
			}

			if (bestMoveTotalPoints <= bestTotalPoints)
			{
				if (bestMoveTotalPoints > bestTotalPoints)
				{
					validMoves.Clear();
				...
				bestTotalPoints = bestMoveTotalPoints;
