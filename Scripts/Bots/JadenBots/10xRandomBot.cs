...
public class TenXRandomBot : BotTemplate
{
    public TenXRandomBot(int botColor)
    {
        color = botColor;
    ...
    public NextMove nextMove()
    {
        ...
        List<NextMove> moves = getAllPossibleBotMovesAndAbilities(this, this.currentBoardState, color);
        ...
        else
        {
            ...
            {
                NextMove move_ = getRandomBotMove(this);

                bool found = false;
                foreach (NextMove nm in allMoves)
                {
                    if (nm.moveType == "move" && move_.moveType == "move")
                    {
                        if (nm.move.p.name == move_.move.p.name)
                        {
                            if (nm.move.coords.x == move_.move.coords.x && nm.move.coords.y == move_.move.coords.y)
                            {
                                found = true;
                                break;
                            }
                    ...
                    if (nm.moveType == "ability" && move_.moveType == "ability")
                    {
                        if (nm.ability.ability == move_.ability.ability)
                        {
                            if (nm.ability.piece.name == move_.ability.piece.name)
                            {
                                found = true;
                                break;
                            }
