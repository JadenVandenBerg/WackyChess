...
public class BotRoss : BotTemplate
{
    ...
    private float evalMove(Design design, NextMove move, BoardState nextMoveBS)
    {
        ...
        {
            coords pos = getPiecePositionFromBoardGrid(nextMoveBS, p);

            float distCount = 0f;
            foreach(int[] coords_ in design.coords)
            {
                float dx = pos.x - coords_[0];
                float dy = pos.y - coords_[1];
                distCount += Mathf.Abs(dx) + Mathf.Abs(dy);

                if (pos.x - coords_[0] == 0 && pos.y - coords_[1] == 0)
                {
                    distCount = 0;
                    break;
                }
