...
public class gameBot : MonoBehaviour
{
    ...
    public void botPieceMoveInvokee()
    {
        if (gameData.turn == -1 && gameData.botMove == false)
        {
            ...
            while (!isMoveable)
            {
                GameObject square = HelperFunctions.generateRandomSquare();
                Piece piece = HelperFunctions.getPieceOnSquare(square);
                if (square != null && piece != null && piece.color == -1)
                {
                    List<int[]> moves = gameData.botMoves[piece];
                    if (moves.Count != 0)
                    {
                        isMoveable = true;
                        ...
                        {
                            gameData.piecesDict.Remove(onSquare.go);
                            Destroy(onSquare.go);
                        }
